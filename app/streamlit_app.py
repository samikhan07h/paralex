...
st.set_page_config(
    page_title="ParaLex AI Legal & Financial Document Assistant",
    page_icon="⚖️",
    layout="wide",
)
...
def get_generator():
    ...
    try:
        ...
        return None, f"Unable to initialize the AI model: {e}"
...
with st.sidebar:
    ...
    st.markdown(
        '<div class="sidebar-label">Document Source</div>',
    ...
    mode = st.radio(
        ...
        help=(
            "Try the built-in sample lease, loan, 10-K, and financial statements, "
    ...
    st.markdown(
        '<div class="sidebar-label">Retrieval Settings</div>',
        unsafe_allow_html=True,
    )
    ...
    top_k = st.slider(
        ...
        help=(
            "Demo Documents defaults to 2 (measured to give the best precision "
            "on the small built-in corpus). Upload Your Own defaults to 4, since "
            "larger uploaded documents need more chunks retrieved to reliably "
    ...
    if feedback_summary["up"] or feedback_summary["down"]:
        with st.expander("Feedback Logged"):
            st.caption(
                f"👍 {feedback_summary['up']}   "
                f"👎 {feedback_summary['down']}"
            )
    ...
    with st.expander("About"):
        st.markdown(
            ...
            "[LinkedIn](https://linkedin.com/in/samikhan07)"
        )
    ...
    if not uploaded_files:
        ...
        if st.session_state.get("_upload_sig") != sig:
            with st.spinner(
                ...
                try:
                    st.session_state["_upload_retriever"] = (
                        build_retriever_from_uploads(
                            uploaded_files,
                            embedder,
                        )
                    ...
                    retriever_status = (
                        "Couldn't process the uploaded file(s). "
                        "Please check that the files are valid PDF/DOCX "
...
st.markdown(
    '<div class="paralex-tagline">'
    "Ask questions about contracts, leases, and financial filings — "
    "every claim is grounded and cited to its source."
    "</div>",
    unsafe_allow_html=True,
)
...
def render_feedback_buttons(
    ...
    rated_ids = st.session_state.setdefault(
        "_rated_message_ids",
        set(),
    )
...
for msg in st.session_state.messages:
    with st.chat_message(
        ...
        st.markdown(
            safe_markdown_text(msg["content"])
        )
        ...
        if msg.get("evidence"):
            with st.expander("Sources & Evidence"):
                ...
                if st.toggle(
                    "Show retrieved text",
                    key=f"show_evidence_{msg['id']}",
                ):
                    cited = extract_cited_source_numbers(
                        msg["content"]
                    )
        ...
        if msg["role"] == "assistant":
            render_feedback_buttons(
                ...
                mode_at_time=msg["mode"],
                top_k_at_time=msg["top_k"],
            )
...
if (
    ...
    with st.chat_message(
        ...
        st.markdown(
            safe_markdown_text(generated.answer)
        )
    ...
    st.warning(
        "Please select or upload a document set before asking a question."
    )
